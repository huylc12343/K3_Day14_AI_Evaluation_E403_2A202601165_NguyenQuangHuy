...
Instructions:
    1. Implement and verify every required evaluation component.
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Compare first score with remaining scores in each result
        first_scores = []
        other_scores = []
        ...
        for result in scores_batch:
            ...
            if len(values) >= 2:
                first_scores.append(float(values[0]))
                other_scores.extend(float(v) for v in values[1:])

        positional_bias = False

        if first_scores and other_scores:
            first_average = sum(first_scores) / len(first_scores)
            other_average = sum(other_scores) / len(other_scores)

            positional_bias = first_average > other_average
...
class BenchmarkRunner:
    ...
    def run(
        ...
        """
        # Pass retrieved contexts through to the retrieval-side metrics and
        # preserve the original pair on the returned result.
        results = []
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_faithfulness = (
            sum(result.faithfulness for result in results)
            / total
        )
        ...
        avg_completeness = (
            sum(result.completeness for result in results)
            / total
        )
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def average(results, attribute):
            if not results:
                return 0.0

            return (
                sum(getattr(result, attribute) for result in results)
                / len(results)
            )
        ...
        if (
            baseline_avg_faithfulness
            - new_avg_faithfulness
            > 0.05
        ):
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Nếu nhiều metric cùng thấp nhất
        lowest_metrics = [
            metric
    ...
    def generate_improvement_suggestions(
        ...
        # Prioritize based on most common failure type
        ...
        for failure_type, _ in sorted_categories:
            ...
            if failure_type == "hallucination":
                ...
                suggestions.append(
                    "Improve prompt instructions and intent routing to keep answers relevant"
                )

            elif failure_type == "incomplete":
                suggestions.append(
                    "Increase context coverage and add few-shot examples to improve completeness"
                )

            elif failure_type == "off_topic":
                suggestions.append(
                    "Improve intent detection and routing to prevent off-topic responses"
                )

            elif failure_type == "refusal":
                suggestions.append(
                    "Review guardrail rules to reduce unnecessary refusals"
                )

            elif failure_type == "unknown":
                suggestions.append(
                    "Review failed cases manually to identify missing failure categories"
                )

        # Requirement: at least 3 suggestions when failures exist
        fallback_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete and grounded answers",
            "Add evaluation guardrails to detect unsupported or irrelevant responses",
        ]
